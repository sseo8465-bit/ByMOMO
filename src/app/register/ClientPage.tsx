...
export default function RegisterClientPage() {
  ...
  const [submitMessage, setSubmitMessage] = useState('');         // 성공/에러 메시지
  const [submitSuccess, setSubmitSuccess] = useState(false);      // 성공 여부
  ...
  const handleSubmit = async () => {
    ...
    if (result.success && !result.error) {
      ...
      // 에러
      setSubmitMessage(result.error || '회원가입 중 문제가 발생했습니다.');
    }
  };
